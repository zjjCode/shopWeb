...
- **表**：`fund_accounts`（`USER_BALANCE`）、`fund_transactions`、`recharge_orders`、`payments`（`biz_type=RECHARGE`） ｜ **流程**：F14（充值，事务 I）、F15（支付/退款，事务 B'/C'）、F6.6（余额支付记账） ｜ **接口**：`/api/balance/*`、`/api/recharges/*`、`/admin/balance/*`、`/admin/recharges/*`、`/api/user/pay-password`（POST 首次设置 / PUT 修改，余额支付前由 `requireBalancePassword` 中间件做二次确认 + Redis 连续失败计数锁定）
...
| `retryRefund` | cron 1min（启动即补偿一次） | 重试 `FAILED` 退款单，指数退避（60s×2^(n-1) 封顶 1h），上限 8 次，命中后转人工 |
